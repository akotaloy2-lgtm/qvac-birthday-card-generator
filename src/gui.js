#!/usr/bin/env node
// QVAC Fake Birthday Card Message For A Coworker Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29462;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/coworkerbirthday") {
      try {
        const { trait } = await readBody(req);
        if (!trait || !trait.trim()) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "Please fill in the field first" }));
          return;
        }
        const result = await generate(modelId, trait.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Birthday Card Message For A Coworker Generator GUI ready at http://localhost:${PORT}`);
  });
